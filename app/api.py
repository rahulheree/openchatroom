...
#     response.set_cookie(
#         key="session_id", value=session_id, httponly=True, secure=True, samesite="lax"
#     )
#     return user
...
#             # Reload the message to get the author relationship loaded
...
async def start_session(response: Response, user_in: schemas.UserCreate, db: AsyncSession = Depends(get_db)):
    ...
    # --- THIS IS THE FIX ---
    # secure=False allows the cookie to be sent over HTTP during development.
    # In a production environment with HTTPS, this should be set to True.
    response.set_cookie(
...
async def restore_session(token: str, response: Response, db: AsyncSession = Depends(get_db)):
    ...
    # Also fix the secure flag here for consistency
